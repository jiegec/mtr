...
long biliip_query(struct bb_biliip *ps, unsigned long ip) {
  ...
  if (i_e >= 1 && ps->index_start[i_e] && ip > ps->index_start[i_e - 1] &&
      ip <= ps->index_end[i_e - 1]) {
    return i_e - 1;
  } else {
    return 0;
  }
            ...
            if (ip_resolve) {
              ...
              if (key != 0) {
                if (biliip->index_ptr[key] == 0) {
                  biliip->ptr = BILIIP_BASE_PTR +
                                (biliip->nCount * 2 + key - 1) * sizeof(int);
                  biliip->index_ptr[key] =
                      htonl(*(int *)(biliip->mmap + biliip->ptr));
                }
                ...
                if (startstat > 70) {
                  if (icountry > 0 && country) {
                    biliip_last +=
                        sprintf(biliip_last, "%.*s", icountry, country);
                  }
                  move(y, startstat - 50);
                  ...
                  *biliip_last = '\0';
                  ...
                  if (iprovince > 0 && province) {
                    biliip_last +=
                        sprintf(biliip_last, " %.*s", iprovince, province);
                    while (biliip_last > biliip_buf &&
                           *(biliip_last - 1) == ' ')
                      *--biliip_last = '\0';
                    printw("%s", biliip_buf);
                    biliip_last = biliip_buf;
                ...
                if (iisp > 0 && isp) {
                  ...
                  while (biliip_last > biliip_buf && *(biliip_last - 1) == ' ')
                    *--biliip_last = '\0';
                }
                if (idesc > 0 && desc) {
